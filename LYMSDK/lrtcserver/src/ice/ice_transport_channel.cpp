...
namespace lrtc
{
    ...
    IceTransportChannel::IceTransportChannel(EventLoop *el, PortAllocator *allocator, const std::string &transport_name,
                                             IceCandidateComponent component) : el_(el),
                                                                                ...
                                                                                ice_controller_(new IceController(this))
    {
        ...
        ping_watcher_ = el_->create_timer_event(ice_ping_cb, this, true);
    }
    ...
    IceTransportChannel::~IceTransportChannel()
    {
        if (ping_watcher_)
        {
            el_->delete_timer_event(ping_watcher_);
    ...
    {
        RTC_DCHECK((!ice_params_.ice_ufrag.empty() || !ice_params_.ice_pwd.empty()));
        // RTC_DCHECK(IceController);
    ...
    void IceTransportChannel::_maybe_start_pinging()
    {
        ...
        if (ice_controller_->has_pingable_connection())
        {
            ...
            // 启动定时器
            el_->start_timer_event(ping_watcher_, cur_ping_interval_ * 1000);
            start_pinging_ = true;
        }
    }
    ...
    void IceTransportChannel::on_check_and_ping()
    {
        ...
        if (cur_ping_interval_ != result.ping_interval)
        {
            ...
            el_->stop_timer_event(ping_watcher_);
            el_->start_timer_event(ping_watcher_, cur_ping_interval_ * 1000);
        }
    }
