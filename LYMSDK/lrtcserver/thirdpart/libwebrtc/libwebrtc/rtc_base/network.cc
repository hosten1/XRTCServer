...
int network_changed = 0;
int64_t last_network_changed_time = 0;
...
void BasicNetworkManager::OnNetworksChanged() {
  ...
  if(network_changed == 2) // disconnected
  {
    network_changed = 0; // unchanged
    UpdateNetworksOnce();
    return;
  }
  network_changed = 1; // connected
  last_network_changed_time = rtc::TimeMillis();
#else
  UpdateNetworksOnce();
#endif
}
    ...
    if (iter == current_networks.end()) {
      ...
      if (include_ignored || !network->ignored()) {
        current_networks[key] = network.get();
         RTC_LOG(LS_INFO) <<"lipan0810-- ConvertIfAddrs network push : "<< network->ToString();
...
void BasicNetworkManager::UpdateNetworksContinually() {
  ...
  if (!sent_first_update_ || (network_changed == 1 && (now-last_network_changed_time) > 2000)) {
    network_changed = 0;
     RTC_LOG(LS_INFO)
        << "BasicNetworkManager::UpdateNetworksContinually: start UpdateNetworksOnce!";
    UpdateNetworksOnce();
  }
#else
  UpdateNetworksOnce();
