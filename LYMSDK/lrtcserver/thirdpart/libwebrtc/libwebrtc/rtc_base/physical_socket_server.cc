...
bool PhysicalSocket::Create(int family, int type) {
  ...
  RTC_LOG(LS_INFO) << "[vrv_wy] PhysicalSocket::Create family=" << family
                   << ",type=" << type << ",result=" << (s_ == INVALID_SOCKET ? "fail." : "succ.");
...
int PhysicalSocket::Bind(const SocketAddress& bind_addr) {
  ...
  RTC_LOG(LS_INFO) << "lipan0809-- Bind  SocketAddress: "<<bind_addr.ToString();
...
// lym 修改 20220225
      ...
      RTC_LOG(LS_INFO) << " 【lym 20220225】 Can't bind socket to network because "
  ...
  RTC_LOG(LS_INFO) << "[vrv_wy] PhysicalSocket::Bind for ::bind() addr=" << bind_addr.ToString();
  RTC_LOG(LS_INFO) << "[vrv_wy] PhysicalSocket::Bind for ::bind() copied_addr=" << bind_addr.ToString() << ",code=" << err;
  if (err < 0) {
#if defined(WIN32) || defined(_WIN32)
	  RTC_LOG(LS_ERROR) << "[vrv_wy] fail wsa_code=" << ::WSAGetLastError();
#else
	  RTC_LOG(LS_ERROR) << "[vrv_wy] fail wsa_info=" << strerror(errno) << ",wsa_code=" << errno;
#endif
  }
...
int PhysicalSocket::DoSendTo(SOCKET socket,
  ...
  //RTC_LOG(LS_INFO) << "[vrv_wy] PhysicalSocket::DoSendTo flags=" << flags << ",code=" << code;
  return code;
